...
// 添加任务
const AddTask: React.FC<AddAndEditTask> = (props: AddAndEditTask) => {
  const { visible, onClose, refresh } = props;
  const [form] = Form.useForm();

  // 发起添加请求
  const { loading, run } = useRequest( (params: Task, isClose: boolean) => ({
    url: '/api/task',
    method: 'POST',
    headers: {
    ...
    >
      <>
        ...
        <div className="form-drawer-button">
          ...
          <Button onClick={() => submit(false)} type="primary" loading={loading}>
            确定
          </Button>
          <Button onClick={() => submit(true)} type="primary" loading={loading}>
            确定并关闭
          </Button>
        </div>
      </>
